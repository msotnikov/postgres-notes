...
0. Спасаем данные из блока построчно
...
5. Останавливаем сервер PostgreSQL

6. бэкапим файлик `/base/16401/16601.296`

7. “Зануляем” весь блок:
```shell
...
8. Проверяем с помощью pg_filedump:
```shell
pg_filedump -i ./base/16401/16601.296 | grep "XMAX: 97"
```

9. Проверяем, с помощью pageinspect:
```
...
10. Генератор команды dd для копирования блоков. Можно искользовать для автоматизации пунктов 1-4 выше.
